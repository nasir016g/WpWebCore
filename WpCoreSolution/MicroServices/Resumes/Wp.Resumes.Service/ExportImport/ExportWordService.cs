...
namespace Wp.Resumes.Services.ExportImport
{
    public class ExportWordService
    {
        ...
        public ExportWordService(ILocalizationService localizationService, ILanguageService languageService)
        {
        ...
        private void InsertExperienceRow(DocX doc, Table table, int rowIndex, string resourceKey, string value, int languageId, Formatting formatting = null)
        {
            if (formatting == null)
            {
                formatting = Normal;
            ...
            row1.Cells[0].InsertParagraph(_localizationService.GetResource(resourceKey, languageId), false, Normal);
        ...
        //private void InsertExperiences(Resume resume, DocX doc, int languageId)
        //{
        //    // 3 experience
        //    doc.InsertParagraph().AppendLine();
        //    doc.InsertParagraph(_localizationService.GetResource("Resume.Fields.Experiences", languageId), false, H2);
        //    //.AppendLine(line);
        ...
        //        InsertExperienceRow(doc, table, 0, "Resume.Fields.Experiences.Name", exp.GetLocalized(x => x.Name), languageId, H4);
        //        InsertExperienceRow(doc, table, 1, "Resume.Fields.Experiences.Period", exp.GetLocalized(x => x.Period), languageId);
        //        InsertExperienceRow(doc, table, 2, "Resume.Fields.Experiences.Function", exp.GetLocalized(x => x.Function), languageId);
        //        InsertExperienceRow(doc, table, 3, "Resume.Fields.Experiences.City", exp.GetLocalized(x => x.City), languageId);
        //        InsertExperienceRow(doc, table, 4, "Resume.Fields.Experiences.Tasks", exp.GetLocalized(x => x.Tasks), languageId);
        ...
        //private void InsertSkills(Resume resume, DocX doc, int languageId)
        //{
        //    doc.InsertParagraph();
        //    doc.InsertParagraph(_localizationService.GetResource("Resume.Fields.Skills", languageId), false, H2);
        //    //.AppendLine(line);                
        ...
        public void ExportResumeToWord(Stream stream, Resume resume, int languageId)
        {
            ...
            using (var doc = DocX.Create(stream))
            {
                ...
                {
                    InsertRowWith2Columns(table, _localizationService.GetResource("Resume.Fields.TelePhone", languageId), resume.Phone);
                } 

                if (!String.IsNullOrWhiteSpace(resume.Mobile))
                {
                    InsertRowWith2Columns(table, _localizationService.GetResource("Resume.Fields.Mobile", languageId), resume.Mobile);
                }
                ...
                doc.InsertParagraph(_localizationService.GetResource("Resume.Fields.Summary", languageId), false, H2).AppendLine();
                //doc.InsertParagraph(resume.GetLocalized(x => x.Summary), false, Normal).AppendLine();

                //InsertEducations(resume, doc);
                //InsertSkills(resume, doc, languageId);
                //InsertExperiences(resume, doc, languageId);
